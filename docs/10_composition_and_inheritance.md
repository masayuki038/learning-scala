...
- オブジェクトの生成は、クライアントに直接newさせず、factoryメソッドを使う
- 1つは、オブジェクトの生成を一箇所に集約すること
- 2つめは、オブジェクトがクラスでどのように表現されているかを隠す
- これらを見せないことによって、クライアントはライブラリをよりシンプルに理解できる
- また、クライアントのコードを壊さずにライブラリのコードを修正しやすくする
