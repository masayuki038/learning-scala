...
- 演算子の優先順位は、評価順序を決める
- 例えば、*は+よりも優先順位が高い為、`2 + 2 * 7`は28ではなく16になる
- これにより、加算よりも積算の方が先にくる
- 括弧を付けることにより、演算の優先順位を上書きすることができる
- Scalaには単体の演算子という概念がないので、演算子の優先順位がどのように作用するか不思議に思うかもしれない
- Scalaは演算記法で呼び出されたメソッドの最初の文字を見て優先順位を決めている(例外は後ほど)
- もしメソッド名が'*'から始まる場合は、'+'から始まるメソッドよりも優先順位が高い
- それゆえ、`2 + 2 * 7`は`2 + (2 * 7))`と評価されるし、`a +++ b *** c`(a、b、cは変数、+++、***はメソッド)は`a +++ (b *** c)`と評価される
- 演算子の優先順位は以下のとおり
...
- +の方が<よりも優先順位が高いので、`2 << 2 + 2`は`2 << 4`となり、結果は32となる
- 同じく、`2 + 2 << 2`は`4 << 2`となり、結果は16となる
- 上記の優先順位の例外としては、=で終わる代入演算子である
- 演算子が=で終わり、比較演算(<=, >=, ==)や否定(!=)でなければ、優先順位は単なる代入(=)と同様である
- それは、他の演算子の優先順位よりも低い。例えば、
...
- `*=`の最初の演算子は`+`より優先順位が高い`*`だけれども、`*=`は`+`より優先順位が下の代入演算子とみなされる
- 式の中に同じ優先順位の演算子を複数並べると、演算子の結合性は演算子がどのようにグルーピングされるかを決める
- Scalaの演算子の結合性は最後の文字で決まる
- `:`で終わるメソッドは左項をパラメータとして右項が起動する
- それ以外のメソッドは右項をパラメータとして左項が起動する
- だから、`a * b`は`a.*(b)`だが、`a:::b`は`b.:::(a)`になる
- しかしながら、演算子がどんな結合性を持っていても、オペランドは左から右に評価される
- なのでもしaが不変値への参照でなければ、` a ::: b`はより厳密には以下のようなブロックになる

```scala
{ val x = a; b.:::(x) }
```

- このブロックは、aはbの前に評価され、その結果はbの:::メソッドに渡される
- この結合性のルールは、同じ優先順位の複数の演算子が並んでいる場合にも役割を果たす
- メソッドが`:`で終わる場合は、右から左にグループ化される
- それ以外は、左から右にグループ化される
- 例えば、`a ::: b ::: c`は`a ::: (b ::: c)`として扱われるが、`a * b * c`は`(a * b) * c`と扱われる
- 演算子の優先順位はScala言語の一部である
