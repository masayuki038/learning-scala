...
# 1.5 Conclusion
- この章では、Scalaがどのようにプログラミングの役に立つかを垣間見てきた
- Scalaは魔法のようにプロダクティブにする銀の弾丸ではない
- 前に進む為に、Scalaにうまく適用していき、学習や実践が必要とされる
- JavaからScalaへの移行は、Scalaを学ぶことは、Scalaの型システムと関数プログラミングのサポートも理解していくことができる
- この本のゴールは一度にワンステップ進めることで、Scalaの学習曲線をなだらかにすることである
- あなたの視野を広げ、プログラム設計について異なった考えをするような、知的な経験があると考えている
- Scalaのプログラミングから喜びとインスピレーションを得られたらなお良い
